...
$sql_num = "select count(1) as num from ads where 1 " . $sql;
...
$sql_data = " select id,name,url from ads where 1 " . $sql . " limit " . ($current_page - 1) * $numPerPage . "," . $numPerPage;
$data = Db::getRows($sql_data);
?>
<div class="page">
    <div class="pageHeader">
        <form onsubmit="return navTabSearch(this);" id="pagerForm" action="index.php?p=admin/ads_list" method="post">
...
foreach($data as $new){
    ...
    echo '<td>'.$new['name'].'</td>';
    echo '<td>'.$new['url'].'</td>';
    echo '<td style="text-align:center" ><a href="index.php?p=admin/ads_add&id='.$new['id'].'" target="navTab" >编辑</a> </td>';
    echo '</tr>';
}
?>
            </tbody>
