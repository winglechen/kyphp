...
class Blockpos
{
    private static $table = 'blockpos';
    public static function add($data)
    {
        $sql = "insert into blockpos (". join(',',array_keys($data)) . ") values(" . Db::addValues($data). ")";
        Db::query($sql);
    }
    ...
    public static function update($data)
    {
        ...
        $sql = 'update blockpos set ' . Db::updateValues($data) . ' where id=' . $id;
        Db::query($sql);
    }
    
    public static function detail($id)
    {
        $sql = 'select * from blockpos where id='.$id;
        return Db::getRow($sql);
    }
    
    public static function delete($id)
    {
        $sql = "delete from blockpos where id=".$id;
        return Db::query($sql);
    }
}
