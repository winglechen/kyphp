<?php
	 ...
	 * 
	 *	@author kj021320
	 *	@email	xinjie.houxj@alibaba-inc.com
	 *	@version 3.15
	 */
	//
		...
		class SecurityLibrary{
			...
			function SecurityPathCheck($file,$filepath=FALSE){
				...
				if(is_array($filepath) ){
					...
					if(!is_string($filepath)){
						die("parameter setting error , file: $f  line:".__line__);
						exit(1);
					}
					if($filepath==""){
						return $file;
			...
			function SecurityPathValidate($file,$filepath=FALSE){
				...
				if($ret===TRUE){
					...
					die("file check error, file:$file  line:".__line__);
					exit(1);
				}
			}
			...
			function DataBaseKeyWord($keyword){
				if(in_array(strtolower($keyword),$this->___DataBaseMetaDataKeyWord)){
					...
					if($this->___DataBaseKeyWordReturn===TRUE){
						...
						die("DataBaseKeyWord check error, KeyWord: $keyword");
						exit(1);
					}
				}
			}
			...
			function __construct($configfile=""){
				...
				if($r===FALSE){
					die("Component init error	line:".__LINE__);
					exit(1);
				}
			}
