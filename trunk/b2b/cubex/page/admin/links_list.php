...
$sql_num = "select count(1) as num from links where 1 " . $sql;
...
$sql_data = " select id,name,url from links where 1 " . $sql . " limit " . ($current_page - 1) * $numPerPage . "," . $numPerPage;
$data = Db::getRows($sql_data);
?>
<div class="page">
    <div class="pageHeader">
        <form onsubmit="return navTabSearch(this);" id="pagerForm" action="index.php?p=admin/links_list" method="post">
...
foreach($data as $new){
    ...
    echo '<td>'.$new['name'].'</td>';
    echo '<td>'.$new['url'].'</td>';
    echo '<td style="text-align:center" ><a href="index.php?p=admin/links_add&id='.$new['id'].'" target="navTab" >编辑</a> | <a   href="index.php?p=admin/links_del&id='.$new['id'].'" target="navTab" onclick="return confirm(\'您确定要删除这条信息吗？\');" >删除</a></td>';
    echo '</tr>';
}
?>
            </tbody>
