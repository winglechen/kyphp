...
class Mnew
{
    private static $table = 'member_new';
    public static function add($data)
    {
        $sql = "insert into member_new (". join(',',array_keys($data)) . ",ts) values(" . Db::addValues($data). ",".time().")";
        Db::query($sql);
    }
    ...
    public static function update($data)
    {
        ...
        $sql = 'update member_new set ' . Db::updateValues($data) . ' where id=' . $id;
        Db::query($sql);
    }
    
    public static function detail($id)
    {
        $sql = 'select * from member_new where id='.$id;
        return Db::getRow($sql);
    }
    
    public static function delete($id)
    {
        $sql = "delete from member_new where id=".$id;
        return Db::query($sql);
    }
}
